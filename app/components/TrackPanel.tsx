...
export default function TrackPanel({ index, category, subtitle, desc, imageSrc }: TrackPanelProps) {
  return (
    <section className="horizontal-panel w-full md:w-screen h-auto md:h-full flex items-center justify-center px-6 md:px-16 lg:px-24 py-12 md:py-0">
      <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
        ...
        <div className="[perspective:1000px] group/tilt">
          <div className="relative aspect-video rounded-lg overflow-hidden border border-white/15 grayscale shadow-2xl transition-transform duration-800 ease-[cubic-bezier(0.16,1,0.3,1)] [transform-style:preserve-3d] w-full group-hover/tilt:[transform:rotateY(-8deg)_rotateX(4deg)_scale(0.98)]">
            <Image
              src={imageSrc}
              alt={category}
              fill
